// Package cli assembles the openlibrary command tree from the openlibrary
...
// NewApp assembles the kit application from the openlibrary domain. The
...
//
// To add a command, declare it in openlibrary/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := openlibrary.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(openlibrary.Domain{}).Register(app)
