...
describes what users get. Pick a format with `-o`, or let openlibrary choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
openlibrary <command> -o table   # aligned columns for reading
openlibrary <command> -o jsonl   # one JSON object per line, for piping
openlibrary <command> -o json    # a single JSON array
openlibrary <command> -o csv     # spreadsheet friendly
openlibrary <command> -o tsv     # tab-separated
openlibrary <command> -o url     # just the URL column
openlibrary <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
openlibrary <command> --fields id,title,url
```
...
```bash
openlibrary <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
openlibrary <command>            # a table, because this is a terminal
openlibrary <command> | wc -l    # JSONL, because this is a pipe
```
